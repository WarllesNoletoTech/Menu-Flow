...
@Schema({ timestamps: true }) export class Restaurant { @Prop({ required: true }) name!: string; @Prop({ required: true, unique: true, lowercase: true, trim: true }) slug!: string; @Prop() logoUrl?: string; @Prop() bannerUrl?: string; @Prop() description?: string; @Prop({ default: false }) blocked!: boolean; @Prop({ default: true }) open!: boolean; @Prop({ type: Object, default: {} }) settings!: Record<string, unknown>; }
...
@Schema({ timestamps: true }) export class Coupon { @Prop({ type: Types.ObjectId, ref: 'Restaurant', required: true, index: true }) restaurantId!: Types.ObjectId; @Prop({ required: true }) code!: string; @Prop({ enum: ['FIXED', 'PERCENTAGE'], required: true }) type!: string; @Prop({ required: true }) value!: number; @Prop({ default: 0 }) minimumOrder!: number; @Prop() startsAt?: Date; @Prop() endsAt?: Date; @Prop() usageLimit?: number; @Prop({ default: true }) active!: boolean; }
...
@Schema({ timestamps: true }) export class RestaurantSettings { @Prop({ type: Types.ObjectId, ref: 'Restaurant', required: true, unique: true }) restaurantId!: Types.ObjectId; @Prop({ type: Object, default: {} }) openingHours!: Record<string, unknown>; @Prop({ default: 0 }) minimumOrder!: number; @Prop() preparationMinutes?: number; @Prop({ default: false }) rappidexEnabled!: boolean; }
